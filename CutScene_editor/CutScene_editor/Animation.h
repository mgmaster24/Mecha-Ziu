...
private:
	...
	unsigned			m_uiCurr;			// cut scene animation indicies
...
public:
	...
	~CInterpolator(void);
	unsigned GetNumFrames(void) { return m_pAnimation->nNumFrames; }
	void GoToFrame(unsigned uiFrame);
	// mutators
	void ToggleAnimation(unsigned int unIndex, bool bActive);
	
	/**********************************************************************
	*	Description: Adds an animation to the list of available animations
	*				 
	**********************************************************************/
	void AddAnimation(unsigned int unStart, unsigned int unEnd, unsigned int unID, bool bLooping);
