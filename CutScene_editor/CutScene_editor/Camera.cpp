...
CCamera::CCamera()
{
	...
	m_vDesiredPositionOffset = D3DXVECTOR3(0.0f, 30.0f, -50.0f);
	m_vLookAtOffset = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
...
{
	m_bChaseMode = false;
	m_pmaPlayer = &CGame::GetInstance()->GetSceneObjects()[0]->GetWorldMatrix();
	...
	if(m_bChaseMode) 
	{
		...
		if(CDirectInput::GetInstance()->CheckKey(DIK_W))
		{
			m_vDesiredPositionOffset.z += 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_S))
		{
			m_vDesiredPositionOffset.z -= 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_A))
		{
			m_vDesiredPositionOffset.x -= 0.5f;
			
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_D))
		{
			m_vDesiredPositionOffset.x += 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_Q))
		{
			m_vDesiredPositionOffset.y -= 0.5f;
		...
		if(CDirectInput::GetInstance()->CheckKey(DIK_NUMPAD4))
		{
			m_vLookAtOffset.x -= 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_NUMPAD6))
		{
			m_vLookAtOffset.x += 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_NUMPAD8))
		{
			m_vLookAtOffset.z += 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_NUMPAD2))
		{
			m_vLookAtOffset.z -= 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_NUMPAD7))
		{
			m_vLookAtOffset.y += 0.5f;
		}
		if(CDirectInput::GetInstance()->CheckKey(DIK_NUMPAD9))
		{
			m_vLookAtOffset.y -= 0.5f;
		}
	}
...
void CCamera::TurnTo(D3DXMATRIX &maMatrix, D3DXMATRIX &maTarget, float fTime, bool bMesh)
{
	...
	{
		D3DXMATRIX result;
		D3DXMatrixRotationX(&result, -fTime * m_fTurnToSpeed);
		maMatrix = result * maMatrix;
	}
...
void CCamera::RestrictLookOnYAxis(float fTime, D3DXMATRIX &maGunMat)
{
	...
	dotResult = D3DXVec3Dot(&worldY, &vGunZ); //0.24, -0.44
	...
	if(deg <= 73.5f)
	{
		m_bRotation = false;
		/*D3DXMATRIX result;
		D3DXMatrixRotationX(&result, fTime);
		maGunMat = result * maGunMat;*/
	}	
	else if(deg >= 133.0f)
	{
		m_bRotation = false;
...
void CCamera::TheDeadZone(D3DXMATRIX *maGunMat, D3DXMATRIX *maPlayerMat, float fDeltaTime)
{
	...
	dotResult = D3DXVec3Dot(&vPlayerX, &vGunZ);
	float angle = acos(dotResult);
	//degrees = Rad * 180 / PI
	float deg = angle * 180 / D3DX_PI;

	bool bLevel2 = false;
	//= CGame::GetInstance()->GetPlayer()->GetLevel2();
	...
	////Round 2 FIGHT
	//dotResult = 0.0f;
	//D3DXVECTOR3 vPlayerY(maPlayerMat->_21, maPlayerMat->_22, maPlayerMat->_23);
	//dotResult = D3DXVec3Dot(&vPlayerY, &vGunZ);
	//angle = acos(dotResult);
	////degrees = Rad * 180 / PI
	//deg = angle * 180 / D3DX_PI;

	//if(deg < 76.0f || deg > 115.0f)
	//{
	//	m_bTurn[1] = true;
	//}
	//else
