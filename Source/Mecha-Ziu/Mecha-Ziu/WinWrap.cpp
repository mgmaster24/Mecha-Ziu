...
{	
	...
	//Load MZ icon
	HICON icon = LoadIcon(hAppInstance, MAKEINTRESOURCE(IDI_ICON1));
	...
	if(!(m_hWnd = CreateWindowEx(					
		...
		(GetSystemMetrics(SM_CXSCREEN)/2) - (windowWidth/2),			// Window Position X in the middle of the screen 
		(GetSystemMetrics(SM_CYSCREEN)/2) - (windowHeight/2),			// Window Position Y in the middle of the screen
