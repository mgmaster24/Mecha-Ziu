...
function Init()
	...
	-- Load the WarTurtle Dialog
	warDiag = LoadDialogue("./Assets/Scripts/BossStart.dlg");
	...
	elseif nLevelDiff == 1 then
		spawnTable = {4, 6, 12, 14, 18};-- Medium
		SetLevelEnemies(58);
	else
...
function Update(fTime)
	...
	if fWaveTime > fNextWaveTime or GetKills() == nTotalSpawn then	
		...
		elseif nLevelDiff == 1 then
			fNextWaveTime = 30 + (1 * spawnTable[nWaveIndex]);
		else
			fNextWaveTime = 45 + (1 * spawnTable[nWaveIndex]);
		end
